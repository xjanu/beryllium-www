...
let pingTime = ""; // ping in milliseconds with 2 decimal digits
...
let settings = {
	...
	count_ping_min: 10, // number of pings to perform in ping test
	time_ping_min: 10,
...
function pingTest(done) {
	...
	const doPing = function() {
		...
		xhr[0].onload = function() {
			...
			pingTime = new Date().getTime() - startT;
		...
		xhr[0].onerror = function() {
			...
			if (settings.xhr_ignoreErrors === 2) {
				//ignore failed ping
				i++;
				if (i < settings.count_ping_min) doPing();
				else {
					// more pings to do?
					pingProgress = 1;
					tlog("ping: " + pingStatus + " jitter: " + jitterStatus + ", took " + (new Date().getTime() - startT) + "ms");
